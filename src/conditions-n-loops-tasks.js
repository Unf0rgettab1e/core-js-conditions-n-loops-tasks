...
function convertToRomanNumerals(num) {
  ...
  let res = '';
  for (let k = 0; k < 5; k += 1) {
    while (culcNum >= romanDict[k][1]) {
      culcNum -= romanDict[k][1];
      res += romanDict[k][0];
    }
  }
  return res;
}
// console.log(convertToRomanNumerals(35));
/**
...
function convertNumberToString(numberStr) {
  ...
  for (let i = 0; i < numberStr.length; i += 1) {
    switch (numberStr[i]) {
      ...
      default:
        res +=
          digitsOrderArr[Number(numberStr[i])] +
...
function isContainNumber(num, digit) {
  let tempNum = num;
  while (tempNum) {
    if (tempNum % 10 === digit) return true;
...
function getBalanceIndex(arr) {
  ...
  for (let index = 1; index < arr.length; index += 1) {
    if (culcSum(0, index - 1) === culcSum(index + 1, arr.length - 1))
      return index;
  }
  return -1;
}
...
function rotateMatrix(matrix) {
  ...
  for (let k = 0; k < matrix.length; k += 1) {
    tempMatrix[k] = [...matrix[k]];
  }
  for (let i = 0; i < matrix.length; i += 1) {
    for (let j = 0; j < matrix.length; j += 1) {
      resultMatrix[j][matrix.length - 1 - i] = tempMatrix[i][j];
    }
  }
...
function sortByAsc(arr) {
  const resultMatrix = arr;
  for (let i = 1, l = arr.length; i < l; i += 1) {
    const current = arr[i];
    let j = i;
    while (j > 0 && arr[j - 1] > current) {
      resultMatrix[j] = arr[j - 1];
      j -= 1;
    }
    resultMatrix[j] = current;
  }
  return resultMatrix;
}
...
function shuffleChar(str, iterations) {
  ...
  const resultPositions = new Array(len);
  for (let i = 0; i < len; i += 1) {
    let newIndex = i;
    for (let k = 0; k < iterations; k += 1) {
      newIndex = newIndex % 2 === 0 ? newIndex / 2 : (len + newIndex - 1) / 2;
    }
    resultPositions[newIndex] = i;
  }

  let resultStr = '';
  for (let i = 0; i < len; i += 1) {
    resultStr += chars[resultPositions[i]];
  }
  return resultStr;
}
